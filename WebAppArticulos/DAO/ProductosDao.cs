            ...
            try
            {
                ...
                try
                {
...

                    {
                        ...
                        producto.ProductoID = rdr.IsDBNull(ID) ? 0 : rdr.GetInt64(ID);
            ...
            try
            {
                ...
                try
                {
                    while (rdr.Read())
                    {
                        ...
                        Models.Categoria categoriaBuscar = categorias.FirstOrDefault(categoria => categoria.CategoriaID == id);
...

                        {
                            categoria = new Models.Categoria
                            {
                                ...
                                CategoriaID = rdr.IsDBNull(ID) ? 0 : rdr.GetInt64(ID),
                                Nombre = rdr.IsDBNull(NOMBRE) ? "" : rdr.GetString(NOMBRE),
                                Eliminado = rdr.IsDBNull(ELIMINADO) ? 0 : rdr.GetInt32(ELIMINADO),
                            };

                            categoria.Producto = new List<Models.Producto>();
                            Models.Producto producto = new Models.Producto();
                            ...
                            producto.FechaCreacion = rdr.IsDBNull(FECHA_CREACION) ? DateTime.MinValue : rdr.GetDateTime(FECHA_CREACION);
                            producto.FechaModificacion = rdr.IsDBNull(FECHA_MODIFICACION) ? DateTime.MinValue : rdr.GetDateTime(FECHA_MODIFICACION);
                            producto.ProductoID = rdr.IsDBNull(PROD_ID) ? 0 : rdr.GetInt64(PROD_ID);
                        ...
                        else
                        {
                            ...
                            if (categoriaBuscar.Producto == null)
                            {
                                categoria.Producto = new List<Models.Producto>();
                            }
